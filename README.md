...
## Carl Ek's Implementation

I implemented it in Python3/Flask  with  MySQL/SQLAlchemy, developed it on Ubuntu 14.04 and Windows 10  
...
1. Install Python3
1. Install packages for flask and sql:  SQLAlchemy, flask, flask_sqlalchemy, pymysql
1. Install any others needed  (os, re, configparser, ...)
1. Unbundle the bundle file to some working directory <dir> 
  * note: I used Pycharm IDE so there might be some cached files in the bundle 
...
CREATE TABLE `payrollfile` (
  `report_id` int(11) NOT NULL,
  `upload_file` varchar(512) NOT NULL,
...
CREATE TABLE `worklog` (
  ...
  `job_group` char(1) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `fk_idx` (`employee_id`),
  KEY `job_group_idx` (`job_group`),
  CONSTRAINT `id` FOREIGN KEY (`employee_id`) REFERENCES `employee` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=241 DEFAULT CHARSET=utf8
```

1. Modify payroll_db.conf with an authorized user and password.


### Running and Testing the Application

1. (if using virtual environment, activate it and use)
1. cd <dir>
1. python <dir>/run.py
1. I included some unit test samples, but more testing should be done.

### API End Points:

1. /  
  * top level (Hello from Flask)
  
1. /payroll
  * payroll portal (Hello from Payroll)
  
1. /payroll/upload
  * upload payroll file
  * optionally display data after upload

1. /payroll/reportid
  * request to display a specific report id
...
Although I don't program with ego, I think I am proud of my simple method to handle bi-weekly totals. I thought far too long on a single query which might do it, thinking there was some magical sql trick or "alchemy", until I realized it could be done in two simple queries with a cleanup for end-of-month day. 

Some other things that could be done: upload employee, date filtering, add unittesting framework, et cetera. 

Since this is a fairly simple application, it might be somewhat bold to say the code is also rather clean and self-explanatory. I will admit I am somewhat new to SQLAlchemy, and this was my first attempt outside of a short tutorial I went over before this assignment.  Getting this working was a small source of pride and satisfaction and I hope you like what you see and can get this installed and running with my instructions.  
